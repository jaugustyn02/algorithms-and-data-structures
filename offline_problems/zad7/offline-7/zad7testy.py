...
def check( G, hint, sol ):
    if sol is None:
        ...
        n = len(G)
        if n < 2: return sol == [0 for _ in range(n)]
        if sol[1] in G[sol[0]][0]: first_gate = gate = 0
        elif sol[1] in G[sol[0]][1]: first_gate = gate = 1
        else: return False
        for i in range(n-1):
        ...
        if (sol[0] not in G[sol[n-1]][gate]) or (sol[n-1] not in G[sol[0]][(first_gate+1)%2]): return False
    return True


def wrong_check( G, hint, sol ):
  # if hint==None :
  if sol is None:
    return hint == sol
  else:
