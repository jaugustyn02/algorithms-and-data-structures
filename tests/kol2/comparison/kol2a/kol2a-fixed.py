...
# 2) Stworzeniu nowej tablicy tylko przystanków przesiadkowych i dodaniu im wartości (liczby przystanków kontrolnych między i-tym a i-1-tym przystankiem)
...
def drivers( P, B ):
    ...
    count = 0
    for i in range(n):
        if P[indices[i]][1] and P[indices[i]][0] <= B:
    ...
    for i in range(3, pp_n):
        J[i] = min(M[i-3:i])
        M[i] = min(J[i-1] + PP[i][1], J[i-2] + PP[i][1] + PP[i-1][1])
    # print(J)
    # print(M)
    if J[pp_n-1] < M[pp_n-1]:
        ...
        who = 1
        curr_count = M[pp_n-1] - PP[pp_n-1][1]
    i = pp_n-1
    result = []
    while i >= 0:
        if who == 0:
