...
# 1) posortowanie każdego słowa względem znaków (count sort)
# 2) posortowanie słów między sobą, najpierw względem długości słowa, następnie alfabetycznie (quick sort)
# 3) przejście liniowe i znalezienie najdłuższego podciągu
# Złożoność czasowa O(NLogN) przy założeniu że quick sort sortuje w czasie O(NlogN)
